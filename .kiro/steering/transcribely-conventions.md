...
4. **Distributables** — Built by CI, never locally. Pushing a `v*` tag triggers `.github/workflows/release.yml`, which gates on the `test` job (unit tests + the live Bedrock check, requires the `MANTLE_API_KEY` secret — historical name, it's a Bedrock API key) and then builds and publishes all three targets: macOS Universal DMG (`--mac --universal`), Windows x64 (`--win --x64`), and Windows ARM64 (`--win --arm64`). Don't run local builds and don't offer to — just confirm the tag was pushed and point at the Actions tab.
