...
- Default model lineup refreshed to current Bedrock flagships: Claude Opus 5 (creator), Claude Sonnet 5 (worker), GPT-6 Sol (formatter), plus Claude Fable 5.1, GPT-6 Astra, and Grok 4.6 available without a role. (Claude Opus 5.5 was pulled from the defaults at the last minute due to an ongoing Bedrock-side serving incident in us-east-1 — add it manually in Settings → Models with `global.anthropic.claude-opus-5-5` once it recovers.) Every default is verified against the live catalog by the integration check.
...
### New — StoryBrand flow ribbon
- **A horizontal ribbon above the script shows the shape of the talk, and doubles as navigation.** One segment per run of consecutive paragraphs sharing an element, in document order, in the same colours as the script. Click a segment to jump to that section, hover for its element, paragraph range, word count and share, and the run you are currently reading stays highlighted as you scroll.
  - Deliberately a sequential ribbon rather than a histogram of element totals. Totals describe composition, which the sidebar's colour bar already shows, and composition throws away the one thing a keynote is made of — order. A ribbon makes the arc visible, and a missing element shows up as an absent colour rather than as something you have to go and check.
  - Segments group consecutive paragraphs into runs rather than drawing one per paragraph: a real keynote is 70-odd paragraphs, which would be 70 unreadable slivers. A 71-paragraph script becomes 8 legible chapters.
  - Width is by word count, not paragraph count, because for a script words are roughly stage time. A 400-word Problem section and a 20-word one are both "one paragraph", and drawing them the same size would misrepresent the talk.
  - Hidden for a script that resolves to a single run — a solid full-width bar implies structure the script does not have.
  - Included in the HTML export, with the colours inlined and the behaviour shipped in the document, so a file shared with someone else carries the same overview and still needs nothing external.
  - Keyboard navigable, and it honours `prefers-reduced-motion`.

### Fixes — StoryBrand
- **The audit panel could not be closed.** Opening it left no way back except restarting the app. The panel is absolutely positioned but no ancestor established a containing block, so it resolved against the whole window and opened as a full-height overlay across the navbar *and* across the Audit button that toggles it. The close button sat at the top of the window where the navbar covered it. Both handlers were correct all along; the layout buried them. Closing no longer depends on a single hit target either — Escape works, and opening and closing now go through one path so the button's state cannot disagree with what is on screen.
- **The StoryBrand sidebar rows were laid out horizontally.** They inherit the shared conversation-row style, which is a centred flex *row*, so the title, the colour bar, the paragraph/word line and the search snippet were all placed side by side: the title was crushed to an ellipsis and **the colour bar collapsed to nothing and had never been visible at all** — the thing meant to let you recognise a keynote by its shape. Rows now stack.
- **The sidebar's rename and delete buttons were unstyled.** They used class names that had no CSS rules behind them, so they rendered as default browser buttons, permanently visible and boxed. They are now a single three-dots button that appears on hover, opening a menu — the same pattern and the same styles as the Work tab, rather than a third description of the same idea.
...
- 20 further tests for the flow ribbon: run grouping, word-count weighting, shares summing to the whole script, unclassified paragraphs kept rather than dropped (dropping them would shift every later segment's position), click-to-jump targeting, the scroll-synced readout, and the export staying self-contained. Mutation-verified: not grouping runs fails 7, sizing by paragraph count fails 3, dropping unclassified paragraphs fails 2, drawing a ribbon for a single run fails 1, jumping without the viewport offset fails 2, and omitting the export's paragraph anchors fails 1.
- One of those tests initially passed against a broken build: it asserted the exported paragraphs carried position anchors by searching the HTML as text, which matched the ribbon's own segments instead. It now parses the document and checks that every segment's jump target exists.
