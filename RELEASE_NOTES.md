...
- **Rework the Transcribe tab, and add naming + search over previous transcriptions.** Tabled deliberately: it is a feature, and the offline-stability work it was discovered alongside is a bug fix that shouldn't wait behind it. Two motivations — (a) a completed job on AWS is currently unreachable from Hive once the renderer moves on, so users re-run transcriptions they have already paid for, and (b) result delivery is tied to the renderer's in-flight `invoke('transcribe-media')` promise, so *any* renderer teardown (credential-expiry navigation, reload, crash) discards a transcript the main process already retrieved successfully.
  ...
  - **Write the transcript JSON to local storage on completion**, next to the registry entry, rather than relying solely on AWS to hold it. Note this is now a *weaker* requirement than when first written: as of v3.5.0 the output bucket is validated and provisioned, so transcripts land durably in a bucket the user owns rather than in a service-managed one. The remaining reasons to keep a local copy are browsing past transcripts **while offline** and not re-fetching from S3 on every view — real, but no longer "the only way not to lose them". (The original note here said Transcribe "hands back a pre-signed `TranscriptFileUri` that expires" — that is the documented behaviour when `OutputBucketName` is *omitted*. Hive was actually sending an empty string, which cannot satisfy the parameter's pattern, so the real symptom was a rejected job. Corrected in v3.5.0.)
  - **Reconcile the registry against `ListTranscriptionJobs`** to backfill jobs from before the registry existed. `transcribe:ListTranscriptionJobs` is already in the set `awsValidator.js` checks for. A guaranteed user-owned output bucket also adds a **second, independent** recovery path that didn't exist before: listing that bucket's objects rebuilds the index even after AWS ages out the job metadata. That path needs **`s3:ListBucket`** on the output bucket, which is *not* currently in the documented permission set (`GetObject`/`PutObject`/`DeleteObject` only — note `ListBuckets` in `awsValidator.js` is the account-level action, a different thing). Add it to the README when this ships.
  - **Output object layout is deliberately flat.** Hive does not set `OutputKey`, so the transcript object is named after the job name — `transcription-<timestamp>.json` at the bucket root. That string is the link between a registry entry and its S3 object. Keeping it flat is a deliberate choice, not an oversight: the job name is already unique and parseable, and structure buys little when the registry holds the metadata. Changing the scheme after the registry ships would require migrating already-transcribed jobs, so revisit only with that cost in mind.
